...
describe("discoverModels — CODEX_MODELS override", () => {
	...
	it("falls back when the override is blank", async () => {
		expect(
			await discoverModels("at-x", fetchJson({}), { CODEX_MODELS: "  , ," }),
		).toBe(FALLBACK_MODELS);
	});
});
...
describe("discoverModels — live /models", () => {
	it("maps visible api-supported models and filters the rest", async () => {
		...
		expect(String(url)).toContain("/models?client_version=0.144.1");
	...
	it("honors the CODEX_CLIENT_VERSION override in the query", async () => {
		...
		await discoverModels("at-x", fetchImpl as unknown as typeof fetch, {
	...
	it("falls back on a non-OK response", async () => {
		expect(await discoverModels("at-x", fetchJson("nope", 500), {})).toBe(
			FALLBACK_MODELS,
		);
	});

	it("falls back on an empty model list", async () => {
		expect(await discoverModels("at-x", fetchJson({ models: [] }), {})).toBe(
			FALLBACK_MODELS,
		);
	});

	it("falls back when the response has no models field", async () => {
		expect(await discoverModels("at-x", fetchJson({}), {})).toBe(
			FALLBACK_MODELS,
		);
	});
